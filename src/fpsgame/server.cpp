...
namespace server
{
    ...
    {
        const bool item = type>=I_SHELLS && type<=I_QUAD;
        const bool ammo = type>=I_SHELLS && type<=I_CARTRIDGES;
        const bool spawn = type==PLAYERSTART;
        return spawn || (!m_noitems && (item && (!m_noammo || !ammo)));
    }
